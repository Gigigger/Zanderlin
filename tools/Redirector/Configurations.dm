...
proc/gen_configs()
		...
		if(line == "\[SERVERS\]")
			...
			if(findtext(line, ".") && !findtext(line, "##"))
				if(server_gen)
					...
					var/serverlink = copytext_char_char(filterline, findtext( filterline, ")") + 1)
					servers.Add(serverlink)
					servernames.Add( copytext_char_char(line, findtext(line, "("), findtext(line, ")") + 1))
	...
	for(var/file in adminfiles)
		...
		for(var/line in admin_config_stream)
			...
			var/akey = copytext_char_char(line, 1, findtext(line, " "))
			adminkeys.Add(akey)
