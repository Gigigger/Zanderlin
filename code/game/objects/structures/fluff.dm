...
/obj/structure/fluff/psycross/attackby(obj/item/W, mob/living/carbon/human/user, params)
	...
	if(!groom_name_index)
		...
		var/last_word = copytext_char(groom.real_name, groom_name_index + 1)
		var/second_last_index = findlasttext(groom.real_name, " ", 1, groom_name_index - 1)

		var/is_title = FALSE
		if(second_last_index)
			var/second_last_word = copytext_char(groom.real_name, second_last_index + 1, groom_name_index)
		...
		if(is_title)
			...
			if(!surname_index)
				surname = " " + copytext_char(groom.real_name, 1, second_last_index)
				groom.change_name("")
			else
			...
			groom.change_name(copytext_char(groom.real_name, 1, groom_name_index))

	var/bride_name_index = findlasttext(bride.real_name, " ")
	var/bride_first_name = bride.real_name

	if(bride_name_index)
		var/last_word_bride = copytext_char(bride.real_name, bride_name_index + 1)
		...
		if(second_last_index_bride)
			var/second_last_word_bride = copytext_char(bride.real_name, second_last_index_bride + 1, bride_name_index)
