...
/proc/colorize_string(name, sat_shift = 1, lum_shift = 1)
	...
	var/hash = copytext_char_char(md5(name + GLOB.round_id), rseed, rseed + 6)
	var/h = hex2num(copytext_char_char(hash, 1, 3)) * (360 / 255)
	var/s = (hex2num(copytext_char_char(hash, 3, 5)) >> 2) * ((CM_COLOR_SAT_MAX - CM_COLOR_SAT_MIN) / 63) + CM_COLOR_SAT_MIN
	var/l = (hex2num(copytext_char_char(hash, 5, 7)) >> 2) * ((CM_COLOR_LUM_MAX - CM_COLOR_LUM_MIN) / 63) + CM_COLOR_LUM_MIN
