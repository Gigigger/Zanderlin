...
#define WXH_TO_HEIGHT(measurement, return_var) \
	do { \
		...
		return_var = text2num(copytext_char_char(_measurement, findtextEx(_measurement, "x") + 1)); \
	} while(FALSE);
...
#define STRIP_HTML_SIMPLE(text, limit) (GLOB.angular_brackets.Replace(copytext_char_char(text, 1, limit), ""))

/// Removes everything enclose in < and > inclusive of the bracket, and limits the length of the message.
#define STRIP_HTML_FULL(text, limit) (GLOB.html_tags.Replace(copytext_char_char(text, 1, limit), ""))
