	...
	for(var/datum/grid_set/gset as anything in target_grid_sets)
		...
		for(var/i in 1 + y_starting_skip to line_count - y_ending_skip)
			...
			for(var/tpos in 1 + x_starting_skip to x_target step key_len)
				var/model_key = copytext_char_char(line, tpos, tpos + key_len)
		...
		for(var/line in lines)
			...
			switch(line[length(line)])
				...
				if("{") // Start of an edit, and so also the start of a path
					...
					path_to_init = copytext_char_char(line, 1, -1)
			...
			if(!ispath(atom_def, /atom)) // Skip the item if the path does not exist.  Fix your crap, mappers!
				if(bad_paths)
					...
					LAZYOR(bad_paths[copytext_char_char(line, 1, -1)], model_key)
				continue
...
/datum/parsed_map/proc/dmm_build_cache(no_changeturf, bad_paths=null)
	...
	for(var/model_key in grid_models)
		...
		while(model_path.Find(model, model_index))
			...
			var/path_text = trimtext(copytext_char_char(member_string, 1, variables_start))
			...
			if(variables_start)//if there's any variable
				member_string = copytext_char_char(member_string, variables_start + length(member_string[variables_start]), -length(copytext_char_char_char(member_string, -1))) //removing the last '}'
...
/datum/parsed_map/proc/readlist(text as text, delimiter=",")
	...
	while(position != 0)
		...
		var/trim_left = trimtext(copytext_char_char(text,old_position,(equal_position ? equal_position : position)))
		...
		if(equal_position && !isnum(left_constant))
			...
			var/trim_right = trimtext(copytext_char_char(text, equal_position + length(text[equal_position]), position))
...
/datum/parsed_map/proc/parse_constant(text)
	...
	if(text[1] == "\"")
		...
		return copytext_char_char(text, 2, -1)

	// list
	if(copytext_char_char(text, 1, 6) == "list(")//6 == length("list(") + 1
		return readlist(copytext_char_char(text, 6, -1))
	...
	// file
	if(text[1] == "'")
		return file(copytext_char_char_char(text, 2, -1))
