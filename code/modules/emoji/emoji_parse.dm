/proc/emoji_parse(text) //turns :ai: into an emoji in text.
	...
	while(1)
		...
		parsed += copytext_char_char(text, pos, search)
		if(search)
			pos = search
			search = findtext(text, ":", pos+1)
			if(search)
				emoji = lowertext(copytext_char_char(text, pos+1, search))
...
/proc/emoji_sanitize(text) //cuts any text that would not be parsed as an emoji
	...
	while(1)
		...
		if(search)
			...
			search = findtext(text, ":", pos+1)
			if(search)
				var/word = lowertext(copytext_char_char(text, pos+1, search))
				if(word in emojis)
					final += lowertext(copytext_char_char(text, pos, search+1))
				pos = search + 1
				continue
		break
	return final
