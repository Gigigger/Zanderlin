...
/proc/load_picture_from_disk(id)
	...
	var/dir = copytext_char_char(pathstring, 1, dir_index)
...
/proc/log_path_from_picture_ID(id)
	...
	switch(mode)
		if("L")
			...
			var/year = copytext_char_char(timestamp, 1, 5)
			var/month = copytext_char_char(timestamp, 5, 7)
			var/day = copytext_char_char(timestamp, 7, 9)
			var/round = data[4]
