...
/mob/proc/brighten_color(input_color)
	...
	if(color_format == 3)
		r_val = hex2num(copytext_char_char(input_color, 1, 2))*16
		g_val = hex2num(copytext_char_char(input_color, 2, 3))*16
		b_val = hex2num(copytext_char_char(input_color, 3, 0))*16
	else if(color_format == 6)
		r_val = hex2num(copytext_char_char(input_color, 1, 3))
		g_val = hex2num(copytext_char_char(input_color, 3, 5))
		b_val = hex2num(copytext_char_char(input_color, 5, 0))
	else
...
/mob/dead/observer/proc/reenter_corpse(forced = FALSE)
	...
	if(istype(src, /mob/dead/observer/profane))
		...
		return
	if(mind.current.key && copytext_char_char(mind.current.key,1,2)!="@")	//makes sure we don't accidentally kick any clients
