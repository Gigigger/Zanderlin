...
/datum/emote/living/custom/proc/check_invalid(mob/user, input)
	. = TRUE
	if(copytext_char_char(input,1,5) == "says")
		to_chat(user, "<span class='danger'>Invalid emote.</span>")
	else if(copytext_char_char(input,1,9) == "exclaims")
		to_chat(user, "<span class='danger'>Invalid emote.</span>")
	else if(copytext_char_char(input,1,6) == "yells")
		to_chat(user, "<span class='danger'>Invalid emote.</span>")
	else if(copytext_char_char(input,1,5) == "asks")
		to_chat(user, "<span class='danger'>Invalid emote.</span>")
	else
		. = FALSE
...
/datum/emote/living/custom/run_emote(mob/user, params, type_override = null, intentional = FALSE)
	if(!can_run_emote(user, TRUE, intentional))
		...
		var/custom_emote = copytext_char_char(sanitize(input("What does your character do?") as text|null), 1, MAX_MESSAGE_LEN)
...
/datum/emote/living/subtle/run_emote(mob/user, params, type_override = null, intentional = FALSE)
	...
	if(is_banned_from(user.ckey, "Emote"))
		...
		var/custom_emote = copytext_char_char(sanitize(input("What does your character subtly do?") as text|null), 1, MAX_MESSAGE_LEN)
		if(custom_emote)
			message = custom_emote
			emote_type = EMOTE_VISIBLE
	else
