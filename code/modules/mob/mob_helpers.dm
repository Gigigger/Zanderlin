...
/proc/stars(n, pr)
	...
	for(var/p = 1 to min(n,MAX_BROADCAST_LEN))
		if ((copytext_char_char(te, p, p + 1) == " " || prob(pr)))
			t = text("[][]", t, copytext_char_char(te, p, p + 1))
		else
			t = text("[]*", t)
...
/proc/slur(n)
	...
	var/newletter=""
	while(counter>=1)
		newletter=copytext_char_char(phrase,(leng-counter)+1,(leng-counter)+2)
		if(rand(1,3)==3)
...
/proc/stutter(n)
	...
	while(p <= n)//while P, which starts at 1 is less or equal to N which is the length.
		var/n_letter = copytext_char_char(te, p, p + 1)//copies text from a certain distance. In this case, only one letter at a time.
	...
	return copytext_char_char(sanitize(t),1,MAX_MESSAGE_LEN)
