...
/obj/abstract/visual_ui_element/console_input/UpdateIcon(appear = FALSE)
	...
	if(cursor_visible && focused)  // Only show cursor when focused
		if(cursor_position >= length(input_text))
			...
			display_text = copytext_char_char(display_text, 1, cursor_position + 1) + "_" + copytext_char_char(display_text, cursor_position + 1)
	else
	...
	if(parts_count == 1)
		...
		for(var/command in base_commands)
			if(findtext(command, current_word, 1, length(current_word) + 1) == 1 && command != current_word)
				prediction_text = copytext_char_char(command, length(current_word) + 1)
		...
		for(var/arg in possible_args)
			if(findtext(arg, current_word, 1, length(current_word) + 1) == 1 && arg != current_word)
				prediction_text = copytext_char_char(arg, length(current_word) + 1)
				showing_prediction = TRUE
				break
	else if(parts_count >= 3)
		var/base_command = input_parts[1]
...
/obj/abstract/visual_ui_element/console_input/proc/handle_keyup(key)
	...
	switch(key)
		if("Backspace", "Back")
			...
			if(cursor_position > 0)
				input_text = copytext_char_char(input_text, 1, cursor_position) + copytext_char_char(input_text, cursor_position + 1)
				cursor_position--
			reset_completion()
			update_prediction()
		if("Delete")
			special_key = TRUE
			if(cursor_position < length(input_text))
				input_text = copytext_char_char(input_text, 1, cursor_position + 1) + copytext_char_char(input_text, cursor_position + 2)
			reset_completion()
		if("Left", "West")
			special_key = TRUE
	...
	if((length(key) == 1 && key != " ") || key == "Space")
		...
		input_text = copytext_char_char(input_text, 1, cursor_position + 1) + char_to_add + copytext_char_char(input_text, cursor_position + 1)
		cursor_position++
		reset_completion()
		update_prediction()
