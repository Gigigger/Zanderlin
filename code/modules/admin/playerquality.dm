/proc/get_playerquality(key, text)
	if(!key)
		return
	var/the_pq = 0
	var/json_file = file("data/player_saves/[copytext_char_char(key,1,2)]/[key]/pq_num.json")
...
/proc/adjust_playerquality(amt, key, admin, reason)
	...
	if(reason || admin)
		...
		thing += "\n"
		text2file(thing,"data/player_saves/[copytext_char_char(key,1,2)]/[key]/playerquality.txt")
...
/proc/check_pq_menu(ckey)
	if(!fexists("data/player_saves/[copytext_char_char(ckey,1,2)]/[ckey]/preferences.sav"))
...
//	dat += "<table width=100%><tr><td width=33%><div style='text-align:left'><a href='?_src_=prefs;preference=playerquality;task=menu'><b>PQ:</b></a> [get_playerquality(user.ckey, text = TRUE)]</div></td><td width=34%><center><a href='?_src_=prefs;preference=triumphs;task=menu'><b>TRIUMPHS:</b></a> [user.get_triumphs() ? "\Roman [user.get_triumphs()]" : "None"]</center></td><td width=33%></td></tr></table>"
	...
	var/list/listy = world.file2list("data/player_saves/[copytext_char_char(ckey,1,2)]/[ckey]/playerquality.txt")
	if(!listy.len)
...
//					usedkey = get_fake_key(C.ckey)
	...
	if(theykey == ckey)
		...
		return
	*/
	if(!fexists("data/player_saves/[copytext_char_char(theykey,1,2)]/[theykey]/preferences.sav"))
...
/proc/add_commend(key, giver)
	if(!giver || !key)
		return
	var/curcomm = 0
	var/json_file = file("data/player_saves/[copytext_char_char(key,1,2)]/[key]/commends.json")
	if(!fexists(json_file))
		WRITE_FILE(json_file, "{}")
	var/list/json = json_decode(file2text(json_file))
	if(json[giver])
...
/proc/get_eslpoints(key)
	if(!key)
		return
	var/curcomm = 0
	var/json_file = file("data/player_saves/[copytext_char_char(key,1,2)]/[key]/esl.json")
	if(!fexists(json_file))
		WRITE_FILE(json_file, "{}")
	var/list/json = json_decode(file2text(json_file))
