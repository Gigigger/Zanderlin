...
/datum/SDQL_parser/proc/select_item(i, list/node)
	...
	else if (copytext_char_char(token(i), 1, 2) == "/")
...
/datum/SDQL_parser/proc/object_type(i, list/node)
	...
	if (copytext_char_char(token(i), 1, 2) != "/")
...
/datum/SDQL_parser/proc/string(i, list/node)
	...
	if(copytext_char_char(token(i), 1, 2) in list("'", "\""))
		node += token(i)
...
/datum/SDQL_parser/proc/value(i, list/node)
	...
	else if(lowertext(copytext_char_char(token(i), 1, 3)) == "0x" && isnum(hex2num(copytext_char_char(token(i), 3))))
		node += hex2num(copytext_char_char(token(i), 3))
		i++
	...
	else if(copytext_char_char(token(i), 1, 2) in list("'", "\""))
		i = string(i, node)

	else if(copytext_char_char(token(i), 1, 2) == "\[") // Start a list.
		i = array(i, node)

	else if(copytext_char_char(token(i), 1, 3) == "@\[")
