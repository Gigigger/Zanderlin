...
/obj/machinery/essence/test_tube/proc/calculate_mixture_color()
	...
	for(var/essence_type in essence_contents)
		...
		total_weight += weight
		var/color_val = hex2num(copytext_char_char(essence.color, 2, 4))
		r += color_val * weight
		color_val = hex2num(copytext_char_char(essence.color, 4, 6))
		g += color_val * weight
		color_val = hex2num(copytext_char_char(essence.color, 6, 8))
		b += color_val * weight
