...
/obj/machinery/light/fueled/cauldron/proc/calculate_mixture_color()
	...
	for(var/essence_type in essence_contents)
		...
		total_weight += weight
		var/color_val = hex2num(copytext_char_char(essence.color, 2, 4)) // R
		r += color_val * weight
		color_val = hex2num(copytext_char_char(essence.color, 4, 6)) // G
		g += color_val * weight
		color_val = hex2num(copytext_char_char(essence.color, 6, 8)) // B
		b += color_val * weight
