...
/proc/color_hex2num(A)
	if(!A)
		return 0
	var/R = hex2num(copytext_char_char(A,2,4))
	var/G = hex2num(copytext_char_char(A,4,6))
	var/B = hex2num(copytext_char_char(A,6,0))
	return R+G+B
...
/proc/color_hex2color_matrix(string)
	...
	var/r = hex2num(copytext_char_char(string, 2, 4))/255
	var/g = hex2num(copytext_char_char(string, 4, 6))/255
	var/b = hex2num(copytext_char_char(string, 6, 8))/255
	var/a = 1
	if(length == 9)
		a = hex2num(copytext_char_char(string, 8, 10))/255
	...
	if(last_slash == 1)
		switch(child)
			...
			if(/area, /turf)
				...
				return /datum
	return text2path(copytext_char_char(string_type, 1, last_slash))
