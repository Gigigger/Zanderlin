...
/datum/controller/subsystem/librarian/proc/get_books(search_title, search_author, search_category)
	...
	for(var/filename in books)
		...
		var/matches = TRUE
		if(search_title && !findtext_char(lowertext(book_info["book_title"]), lowertext(search_title)))
			matches = FALSE
		if(search_author && !findtext_char(lowertext(book_info["author"]), lowertext(search_author)))
			matches = FALSE
