...
/proc/getleftblocks(input,blocknumber,blocksize)
	if(blocknumber > 1)
		return copytext_char_char(input,1,((blocksize*blocknumber)-(blocksize-1)))

/proc/getrightblocks(input,blocknumber,blocksize)
	if(blocknumber < (length_char(input)/blocksize))
		return copytext_char_char(input,blocksize*blocknumber+1,length_char(input)+1)

/proc/getblock(input, blocknumber, blocksize=DNA_BLOCK_SIZE)
	return copytext_char_char(input, blocksize*(blocknumber-1)+1, (blocksize*blocknumber)+1)
